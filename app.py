...
os.environ["SPOTIPY_CLIENT_ID"] = '13bc7d9c330c4f2a8049b1ce3e7a84ca'
os.environ["SPOTIPY_CLIENT_SECRET"] = 'b767c2d01ded49e3a94927e308b305ca'
os.environ["SPOTIPY_REDIRECT_URI"] = 'http://localhost:8888/callback/'

scope = 'user-library-read'
username = 'klqbfo7ijoh9b5ueiys5p7qs2'

token = spotipy.util.prompt_for_user_token(username, scope)

if token:
    spotipy_obj = spotipy.Spotify(auth=token)
    saved_tracks_resp = spotipy_obj.current_user_saved_tracks(limit=5)
else:
    print('Couldn\'t get token for that username')

number_of_tracks = 5#saved_tracks_resp['total']
...
pd.set_option('display.max_rows', 5) #len(tracks)
...
rec_df = pd.read_csv('recommended_songs.csv')
...
st.title('Real-Time Music Recommendation using PySpark and Kafka')

col1,col2 = st.columns(2)
col1.header("Your Recommended Songs")


df2 = rec_df[['name','artists']]
print(df2)
col1.table(df2.head(5))
#


col2.header("Features of your Recommended Songs")
df3 = rec_df.loc[:5, ['name', 'artists', 'acousticness', 'liveness', 'instrumentalness', \
...
#
col3.header("Your Latest Added Song")
top_5_songs = df[['name', 'artists']].head(5)
col3.table(top_5_songs)
#
col4.header("Your Top 5 Artists")
df1 = df['artists'].value_counts()[:5].to_frame()
df1['Name'] = df1.index
...
col5.header("Your Favourite Artist")
col5.markdown(f'<p class="big-font">{str(ur_favourite_artist)}</p>', unsafe_allow_html=True)
#
col6.header("Total Time of Songs")
time = round(df.duration_s.sum() / 3600, 2)
col6.markdown(f'<p class="big-font">{round(df.duration_s.sum() / 3600, 2)} hours</p>', unsafe_allow_html=True)
#
col7.header("Total Number of Songs")
col7.markdown(f'<p class="big-font">{df.count()[1]} songs</p>', unsafe_allow_html=True)
